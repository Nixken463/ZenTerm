...
base_parser.add_argument(
    "-f", "--force", action="store_true", help="Suppress errors and force the operation"
)
base_parser.add_argument(
    "-i",
    "--interactive",
    action="store_true",
    help="Prompt for confirmation before each action",
)
base_parser.add_argument(
    "-v",
    "--verbose",
    action="store_true",
    help="Provide detailed output during execution",
)

rm_parser = argparse.ArgumentParser(parents=[base_parser])
rm_parser.add_argument(
    "-r", "--recursive", action="store_true", help="Recursively delete directories"
)
...
class ZenShell(Cmd):
    ...
    def do_mkdir(self, arg):
        ...
        parser.add_argument(
            ...
            "--parents",
            action="store_true",
            help="create parent directory if not existing",
        )
        parser.add_argument(
            "path", default=".", help="Location and name of the directory to create"
        )
    ...
    def do_rm(self, args):
        def confirm(path):
            resp = (
                input(
                    f"rm: remove {'directory' if path.is_dir() else 'file'} '{path}'? (y/n) "
                )
                .strip()
                .lower()
            )
    ...
    def do_ls(self, arg):
        parser = argparse.ArgumentParser(parents=[base_parser])
        parser.add_argument(
            "path", nargs="?", default=".", help="Path to the ls location"
        )
        try:
            parse = parser.parse_args(arg.split())
        except SystemExit:
